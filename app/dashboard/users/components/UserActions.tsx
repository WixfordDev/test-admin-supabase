...
export default function UserActions({ user, onUserUpdate, onDelete }: UserActionsProps) {
  ...
  const sendNotification = async (
    ...
    try {
      const res = await fetch('/api/send-announcement', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ title, body, user_id: userId, type }),
      })
      const data = await res.json()
      if (!res.ok) console.error('Notification error:', data.error)
      else console.log('✅ Notification sent & saved:', data)
    } catch (e) {
      console.error('Notification error:', e)
    }
  }
  ...
  const handleSubscriptionChange = async (newStatus: SubscriptionStatus) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // ✅ type: 'subscription' পাঠানো হচ্ছে
        await sendNotification(user.user_id, title, body, 'subscription')
  ...
  const handleSuspendUser = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // ✅ type: 'warning' পাঠানো হচ্ছে
        await sendNotification(user.user_id, '⚠️ Account Suspended', body, 'warning')
  ...
  const handleSendCustomMessage = async () => {
    ...
    setSendingMsg(true)
    try {
      // ✅ type: 'announcement' পাঠানো হচ্ছে
